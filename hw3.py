...
def kNearestN(dat, lab, test, K):
    ...
    for m in range(len(test)):
        ...
        lookAt = dis_lab[:K] #Select only k neighbors to look at
...
class TitanicData():
    ...
    def import_clean_TitanicData(self):
        ...
        test = wholeData.loc[~wholeData.index.isin(train.index)] #Take the rest of wholeData that was Not used in train and uses those unuses values to create test
            #takes all of the samples that are in wholeData but are NOT in train
        #print(train.columns.values) #Prints the values as nparray format
        
        dev = test.sample(frac = 0.5) #dev is 50% of test data ... AKA 15% of original dataset
        
        test = test.loc[~test.index.isin(dev.index)]  
        ...
        dev_labels = dev["Survived"] #Get labels for dev from test data
        dev.drop("Survived", inplace=True, axis=1)
 
        test_labels = test["Survived"] #Update test data to be 15% of original dataset
