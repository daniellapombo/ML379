...
class NLP_:
    ...
    def unique_words(self, splt, txt_dat, n, m):
        ...
        #print('')
        #print('First 10 unique words', unique_words)
        #print('')
        
        unq_w_dic = dict() #Dictionary of counts (number of appearances) of uniqe words in the data 
        #Keys are the counts (freqency) and values are list of words that appear that many times w/ in data
        ...
        for key in unique_words:
            ...
            if n <= doc_freq <= m:
                ...
                if key in unq_w_dic:
                    new_val = unq_w_dic[doc_freq] #Old value for key
                    new_val.append(doc_freq) #Updates value for key
                    unq_w_dic[doc_freq] = new_val #Assigns new list of words w/ that frequnecy to the key 
                    
                    
                else:
                    unq_w_dic[doc_freq] = [key]
        ...
        print('Dictionary of unique words', unq_w_dic)
        
        return unq_w_dic, list_unq
    ...
    def elem_freq(self, txt_dat, unique_words, sparse_index):
        ...
        for txt in txt_dat: #Iterate over data
            for key in unique_words: #Iterate over key words
                ...
                #print('')
                #if count >0:
                    #print('count', count, 'key', key, 'text:', txt)
                    #print('')
            ...
            #print('bag of words text i',b_of_w[i, :] )
            i += 1 #Increment index
    ...
    def elem_unigram(self, txt_dat, n, m, if_FinalTrain = False, if_TestSet = False):
        ...
        if not if_FinalTrain and not if_TestSet:
            unq_freq, unq_list = self.unique_words(split, txt_dat, n, m)
        ...
        if if_FinalTrain:
            self.unq_freq, self.unq_list = self.unique_words(split, txt_dat, n, m)
...
def main():
    ...
    run_.elem_unigram(None, 3, 500, True, True)
